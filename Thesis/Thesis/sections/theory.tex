...
\section{Machine Learning Theory} \label{section::theory}
 ...
 Afterwards, the backpropagation algorithm~\ref{subsection::backpropagation} and BPTT~\ref{subsection::bptt} are described at a very basic level.
 ...
 \subsection{Image-Prediction / Video-Prediction} \label{subsection::imageprediction}
  Image-/Video-prediction is a field inside machine learning and computer vision, where the key is to predict future images, given a sequence of images. The image 
  sequence $X$ is of length $n$, ($x_0, \ldots, x_{n-1}$).
  \\\\
  ...
  ground-truth is missing. The predicted frames are only approximated, which means they contain a certain level of error. Using them as input in a
 ...
 \subsection{Autoencoder} \label{subsection::autoencoder}
  ...
  image, which the architecture should reconstruct.
 ...
 \subsection{CNN} \label{subsection::cnn}
  ...
  \begin{enumerate}
   ...
   \item Pooling layer
  \end{enumerate}
  More precisely, the used convolutional operation is the discrete convolutional operation.  
  ...
  \begin{figure}[H]
   ...
   \label{fig:kernel}
  \end{figure}\noindent
  In figure~\ref{fig:kernel}, one can see the idea of applying the kernel on the input data (The Padding\footnote{Padding is the idea of adding layers of zeros
  to the current image, so the kernel has more space to cover the image.} size is zero and the stride\footnote{Stride is the parameter of movement of the kernel 
  through the image.} is one.).\\
  The second stage is applying a non-linear function on the output, e.g. ReLU:
  \begin{equation}
   R(x) = max(0, x)
  \end{equation}
  Lastly there is the pooling layer, in which a pooling function is applied on the output of the second stage. This is done to aggregate the output and to make 
  the output invariant to the input.\\
  The standard pooling nowadays is Max-Pooling \cite{Zhou1988}. The idea is simple, as using a kernel with a given size and
  and a stride over the output of the second stage, and take always only the highest value within the given area, the kernel currently inspects.
  \begin{figure}[H]
   \includegraphics[width=0.6\textwidth]{../Images/max_pooling.png}
   \centering
   \caption{Simple example of Max-Pooling (Filter $:=$ Kernel)\cite{savyakhosla2019}}
   \label{fig:max_pooling}
  \end{figure}\noindent
  The pooling layer reduces the output size by at least two. For example, an input image of size $(32 \times 32 \times 1)$,
 ...
 \subsection{RNN} \label{subsection::rnn}
  ...
  of the RNN of the last time-step. This requires a RNN to be initialized at start, because there is no output of the last time-step available. This last time-step input is often initialized as zero.
 ...
 \subsection{Backpropagation} \label{subsection::backpropagation}
  ...
  The algorithm should be already known to the reader, therefore this thesis will only give a very simple example of it at the most simple neural network, the 
  Perceptron \cite{Rosenblatt1957}.
  \begin{center}
  ...
  \end{equation}
  $\sigma$ is the sigmoid function, which is defined as:
  \begin{equation}
